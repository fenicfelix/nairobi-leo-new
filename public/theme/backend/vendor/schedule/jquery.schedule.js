  ...
  $.extend(Plugin.prototype, {
    ...
    add: function (parent, position, height, options) {
      ...
      var period = $('<div class="jqs-period">' +
        '<div class="jqs-period-container">' + periodTitle + periodTime + periodRemove + periodDuplicate + '</div>' +
        '</div>').css({
          'top': position * this.periodPosition,
          'height': height * this.periodPosition
    ...
    openOptions: function (event, period) {
      ...
      var titleInput = '<div class="jqs-options-title-container my-3">' +
        '<select class="form-control jqs-options-show" name="jqs-options-show">' +
        '<option value="">Select Show</option>';
      $.each(shows, function (index, value) {
        titleInput += '<option value="' + value.id + '">' + value.title + '</option>';
      });
      titleInput += '</select></div>';
      ...
      $('.jqs-options-color', this.element).click(function () {
        $('.jqs-period-container', period);
      });

      $(".jqs-options-show", this.element).change(function () {
        console.log($(".jqs-options-show").val());
        $.each(shows, function (index, value) {
          if (value.id == $(".jqs-options-show").val()) {
            $('.jqs-period-title', period).text(value.title);
            period.attr('id', value.id);
            period.attr('title', value.title);
            //break;
          }
        });
      });
